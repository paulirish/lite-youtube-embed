	...
	class LiteYTEmbed extends HTMLElement {
		connectedCallback() {
			...
			this.posterUrl = posterImage
				? posterImage
				: backgroundImage !== ''
				? LiteYTEmbed.getBackgroundImage(backgroundImage)
		...
		static getBackgroundImage(backgroundImage) {
			const [match] = backgroundImage.match(/url(("|')?.*("|'))\)/g);
			const isQuotes = match.indexOf('"') > 0;
			const url = isQuotes
				? match.slice(match.indexOf('"') + 1, -2)
				: match.slice(match.indexOf("'") + 1, -2);
